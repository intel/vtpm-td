...
impl SpdmConnection {
    ...
    fn provision_tpm(&self) -> VtpmResult {
        if !GLOBAL_TPM_DATA.lock().ca_cert_ready() {
            log::error!("vtpm ca_cert is not ready! ek-provision is skipped.\n");
    ...
    pub fn run(&mut self) -> VtpmResult {
        ...
        if provision_info.is_none() {
            ...
            return Err(VtpmError::SpdmError);
        }
        ...
        loop {
            ...
            if operation.is_ok() && operation.unwrap() == TdVtpmOperation::Destroy {
                log::info!("Destroy event received.\n");
                return Ok(());
            }
        }
    }
