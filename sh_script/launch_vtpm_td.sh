...
process_args() {
    ...
    QEMU_CMD="$QEMU \
            ...
            -smp 1 -m ${MEM} \
            -object tdx-guest,id=tdx,debug=on,vtpm-type=server,quote-generation-service=vsock:1:4050,vtpm-userid=${USERTD_ID},vtpm-path=unix:/tmp/vtpm-server-${USERTD_ID}.sock \
